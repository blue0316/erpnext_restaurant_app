class ProductItem {
    ...
    make_fields() {
        ...
        this.search_field.$input.on('input', (e) => {
            ...
            this.last_search = setTimeout(() => {
                ...
                item_group === "" ? "All Item Groups" : item_group;
        ...
        this.item_group_field = frappe.ui.form.make_control({
            df: {
                ...
                onchange: async () => {
                    let item_group = this.item_group_field.get_value();
                    item_group === "" && (item_group = "All Item Groups");

                    this.item_groups = await this.get_group_items(item_group);
                    
                    if(this.item_groups.length > 0) {
                        this.render_group_items();
                    } else {
                        let items = await this.get_items({item_group: item_group});
    ...
    async get_group_items(item_group) {
        const result = await frappe.db.get_list("Item Group", {
            filters: {
    ...
    get_group_item_html(item) {
        ...
        return frappe.jshtml({
            ...
            this.filter_items({ item_group: name })
        }).html();
        ...
        function template() {
            ...
            <div class="product-img"> ${image ?
                ...
                `<span class="placeholder-text" style="font-size: 72px; color: #d1d8dd;"> ${frappe.get_abbr(name)}</span>`}
                ${
                    item.price_list_rate ?
                    `<span class="price-tag">
                        ${price_list_rate}
    ...
    render_group_items() {
        const raw_item_groups = this.item_groups.map(item => this.get_group_item_html(item));
        
        raw_item_groups.reduce((acc, item) => acc += item, '');
    
        this.clusterize.update(raw_item_groups);
    }
    ...
    render_items(items) {
        const raw_items = Object.values(items || this.item_groups).map(item => {
    ...
    async filter_items({ search_term = '', item_group = this.parent_item_group } = {}) {
        const result_arr = [];
        if (search_term) {
        ...
        item_group === "" && (item_group = "All Item Groups");
        this.item_groups = await this.get_group_items(item_group);
        if(this.item_groups.length) {
            this.render_group_items();
        } else {
            this.items = await this.get_items({item_group: item_group});
    ...
    async load_items_data() {
        this.item_groups = await this.get_group_items("All Item Groups");
